...
// Internal array size covering ASCII printable characters.
const OPT_INDEX_SIZE: usize = 128;
...
pub struct ParsedOpts {
    // Fixed-size arrays indexed by ASCII character code for O(1) access.
    counts: [u32; OPT_INDEX_SIZE],
    values: [Vec<String>; OPT_INDEX_SIZE],
}
...
struct OptSpec {
    // `exists[ch]` is true iff `ch` is a recognised option.
    exists: [bool; OPT_INDEX_SIZE],
    // `has_arg[ch]` is true iff the option takes an argument.
    has_arg: [bool; OPT_INDEX_SIZE],
    groups: Vec<Vec<char>>,
    min_args: usize,
    max_args: usize,
...
fn parse_optstr(optstr: &str) -> Result<OptSpec, String> {
    let mut exists = [false; OPT_INDEX_SIZE];
    let mut has_arg = [false; OPT_INDEX_SIZE];
        ...
        match c {
            ...
            '(' => {
                ...
                if i < chars.len() && chars[i] == ')' {
                    ...
                    return Err("unmatched '(' in option string".to_string());
                }
            }
            '?' => {
            ...
            ch if ch.is_ascii_alphanumeric() => {
                let opt_char = ch;
                let mut arg = false;
                i += 1; // consume the letter/digit
                if i < chars.len() && chars[i] == ':' {
                    arg = true;
                    i += 1; // consume the ':'
                }
                let idx = opt_char as usize;
                if idx < OPT_INDEX_SIZE {
                    exists[idx] = true;
                    has_arg[idx] = arg;
                }
            }
    ...
    // Second pass: process parentheses content without extra allocations.
    for (start, end) in paren_pairs {
        let inner = &optstr[start + 1..end]; // direct slice, no String allocation
        if inner.len() == 1 {
            let ch = inner.chars().next().unwrap();
            if ch.is_ascii_alphanumeric() {
                let idx = ch as usize;
                if idx < OPT_INDEX_SIZE && !exists[idx] {
                    exists[idx] = true;
                    // Long alias is a plain option without argument.
                    has_arg[idx] = false;
                }
            }
        } else if inner.len() > 1 {
            ...
            let all_opts = inner.chars().all(|ch| {
                ch.is_ascii_alphanumeric() && (ch as usize) < OPT_INDEX_SIZE && exists[ch as usize]
            });
            if !all_opts {
                // Long name – ignore.
            }
...
pub fn parse(ctx: &mut Context, optstr: &str) -> Result<ParsedOpts, String> {
    ...
    let mut counts = [0u32; OPT_INDEX_SIZE];
    let mut values: [Vec<String>; OPT_INDEX_SIZE] = std::array::from_fn(|_| Vec::new());
    ...
    while let Some(arg) = args.next() {
        ...
        if arg_str == "--" {
            ...
            positional.reserve(args.len());
            for remaining in args {
                positional.push(remaining.to_string());
            }
            break;
        }
        ...
        if arg_str.starts_with('-') && arg_str.len() > 1 {
            ...
            while i < opt_chars.len() {
                let ch = opt_chars[i];
                let idx = ch as usize;
                if idx >= OPT_INDEX_SIZE || !spec.exists[idx] {
    ...
    for group in &spec.groups {
        let used: u32 = group.iter().map(|&ch| counts[ch as usize]).sum();
        if used > 1 {
